...
public class AStarSearch
{
	...
	public static void search(int[] board, boolean d, char heuristic)
	{
		...
		while (!q.isEmpty()) // while the queue is not empty
		{
			...
			{
				System.out.println("inside temp node");
				...
				for (int i = 0; i < tempSuccessors.size(); i++)
				{
					...
					if (heuristic == 'o')
					{
						...
						System.out.println(tempSuccessors.get(i).findCost());
						System.out.println(((EightPuzzleState) tempSuccessors.get(i)).getOutOfPlace());
						checkedNode = new SearchNode(tempNode,tempSuccessors.get(i), tempNode.getCost() + tempSuccessors.get(i).findCost(),
